 ...
 * -----
 *
 * @key `/{path}` - A relative path to an individual endpoint
 ...
 * @key `externalValue` - Optional A URI that points to the literal example
 * @key `x-${string}` - Specification Extensions
 *
 * @note
 ...
 * @key `explode` - Optional When this is true, parameter values of type array or object generate separate parameters for each value
 ...
 * @key `example` - Optional Example of the parameter's potential value
 * @key `examples` - Optional Examples of the parameter's potential value
 ...
 * code to the expected response. The Responses Object MUST contain at least one response code,
 * and it SHOULD be the response for a successful operation call.
 *
 * | Version | Reference |
 * |---|-----|
 * | 3.1.1   | {@link https://spec.openapis.org/oas/v3.1.1#responses-object | OpenAPI 3.1.1 Responses Object} |
 * | 3.1.0   | {@link https://spec.openapis.org/oas/v3.1.0#responses-object | OpenAPI 3.1.0 Responses Object} |
 *
 * -----
 * Fields
 * -----
 *
 * @key `{statusCode}` - A HTTP status code or range of status codes
 * @key `default` - The documentation of responses other than the ones declared for specific HTTP response codes
 *
 * @note
 * The Responses Object MUST contain at least one response code. The `default` key can be used
 * to define a default response for all HTTP codes that are not covered individually by the specification.
 *
 * -----
 * Examples
 * -----
...
export interface Responses {
  /**
   * A HTTP status code or range of status codes. The key can be a specific status code
   * (e.g., "200", "404") or a range (e.g., "2XX", "4XX"). The value describes the response
 ...
 * -----
 *
 * @key `description` - Optional A brief description of the header
 * @key `required` - Optional Determines whether this header is mandatory
 * @key `deprecated` - Optional Specifies that a header is deprecated
 * @key `allowEmptyValue` - Optional Sets the ability to pass empty-valued headers
 * @key `style` - Optional Describes how the header value will be serialized
 * @key `explode` - Optional When this is true, header values of type array or object generate separate headers for each value
 * @key `allowReserved` - Optional Determines whether the header value SHOULD allow reserved characters
 * @key `schema` - Optional The schema defining the type used for the header
 * @key `example` - Optional Example of the header's potential value
 * @key `examples` - Optional Examples of the header's potential value
 * @key `content` - Optional A map containing the representations for the header
 * @key `x-${string}` - Specification Extensions
 *
 * @note
 ...
 * -----
 *
 * @key `schema` - Optional The schema defining the content of the request, response, or parameter
 ...
 * @key `explode` - Optional When this is true, property values of type array or object generate separate parameters for each value
 * @key `allowReserved` - Optional Determines whether the parameter value SHOULD allow reserved characters
 * @key `x-${string}` - Specification Extensions
 *
 * @note
 ...
 * -----
 *
 * @key `{expression}` - A Path Item Object or Reference to a Path Item Object
 *
 * @note
